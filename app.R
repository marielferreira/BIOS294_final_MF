...
server <- function(input, output) {
  ...
  data_clean <- reactive({
    ...
    df <- read.csv(input$file$datapath, header = TRUE) # Read the file that was uploaded, I use df because that is easiest for me and I wont mess up later with the code
  ...
  output$plot_description <- renderUI({
    description <- switch(input$plot_type,
    ...
    HTML(paste("<b>Plot Description:</b><br>", description)) #formats and returns HTML text, shows bold heading
  })
...
#I have a good amount of experience with R but I also used resources such as listed below: 
