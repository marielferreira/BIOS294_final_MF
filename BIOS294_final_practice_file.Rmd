...
# Load and clean data
df <- read.csv("C:\\Users\\Mariel Ferreira\\Downloads\\final_data.csv", header = TRUE) #I use df because that is easiest for me and I wont mess up later with the code
...
# 1. Reorient data to compare Sinclair scores across regions
sinclair_long <- data_clean %>%
  pivot_longer(cols = starts_with("Sinclair_"),
               names_to = "Scalp_Region", values_to = "Sinclair_Score")

# 2. Reorient data to compare Hair Thickness across regions
thickness_long <- data_clean %>%
  pivot_longer(cols = starts_with("Hair_Thick_"),
               names_to = "Scalp_Region", values_to = "Hair_Thickness")

# 3. Correlation model between Sinclair and Hair Count
lm_model <- lm(Hair_Num_Vert ~ Sinclair_Vert, data = data_clean)
summary(lm_model)
model_coef <- tidy(lm_model)

# 4. Visualize hair patterns
hair_long <- data_clean %>%
  pivot_longer(cols = starts_with("Hair_Num_"),
               names_to = "Region", values_to = "Hair_Count")

ggplot(hair_long, aes(x = Region, y = Hair_Count, fill = Region)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Hair Count Distribution by Region")

# Violin plot
ggplot(hair_long, aes(x = Region, y = Hair_Count, fill = Diagnosis)) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  theme_minimal() +
  labs(title = "Violin Plot: Hair Count by Region and Diagnosis")

# Faceted Sinclair scores by diagnosis
ggplot(sinclair_long, aes(x = Scalp_Region, y = Sinclair_Score, group = SampleID)) +
  geom_line(alpha = 0.5) +
  facet_wrap(~ Diagnosis) +
  labs(title = "Sinclair Score Patterns Across Scalp Regions by Diagnosis") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  theme_minimal()

# Scatterplot of Sinclair vs Hair Thickness
ggplot(data_clean, aes(x = Sinclair_Vert, y = Hair_Thick_Vert, color = Diagnosis)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  theme_minimal() +
  labs(title = "Sinclair Score vs Hair Thickness (Vertex)")
...
ggplot(model_data, aes(x = .fitted, y = .resid)) +
  ...
  labs(title = "Residuals vs Fitted Values", x = "Fitted Hair Count", y = "Residuals") +
  theme_minimal()

# Bar graph - Diagnosis frequency
